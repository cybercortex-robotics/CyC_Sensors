...
static float ACC_N = 0.08f;     // accelerometer measurement noise standard deviation. #0.2   0.04
static float GYR_N = 0.004f;    // gyroscope measurement noise standard deviation.     #0.05  0.004
static float ACC_W = 0.00004f;  // accelerometer bias random walk noise standard deviation.  #0.02
static float GYR_W = 2.0e-6f;   // gyroscope bias random walk noise standard deviation.     #4.0e-5
...
public:
        ...
        const Eigen::Vector3d& linearized_ba, const Eigen::Vector3d& linearized_bg)
    {
        ...
        midPointIntegration(_dt, _acc_0, _gyr_0, _acc_1, _gyr_1, delta_p, delta_q * Eigen::Quaterniond(1, turb(0) / 2, turb(1) / 2, turb(2) / 2), delta_v,
            linearized_ba, linearized_bg,
            turb_delta_p, turb_delta_q, turb_delta_v,
            turb_linearized_ba, turb_linearized_bg, 0);
